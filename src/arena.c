...
dev_mm_alloc()
{
	...
	if (mm == NULL) {
		size_t idx = __atomic_fetch_add(&dev_n_mapped_mem, 1, __ATOMIC_RELAXED);
...
bool
dev_is_gmm(const void *ptr)
{
	if ((uintptr_t)ptr >= (uintptr_t)dev_mapped_mem &&
			(uintptr_t)ptr < (uintptr_t)(dev_mapped_mem + DEV_MM_COUNT)) {
		return true;
	}
